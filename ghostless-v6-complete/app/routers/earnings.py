...
async def accept_task(
    ...
    # ── FIX #6: Payout MUST be coupled to trust * confidence ──────────────
    # Earnings formula: payout = base_payout * (trust / 100) * confidence_weight
    # This means low-trust and low-confidence workers earn less per task.
    # No exceptions — trust and confidence gate the actual payout.
    if body.accepted:
        base_payout = Decimal(str(body.payout_override_usd)) if body.payout_override_usd else task.payout_amount
        # Fetch worker score for trust and confidence
        score_result = await db.execute(
            select(WorkerScore)
            .join(Worker, Worker.id == WorkerScore.worker_id)
            .where(Worker.id == task.worker_id)
            .where(WorkerScore.tenant_id == auth.tenant_id)
        )
        score = score_result.scalar_one_or_none()
        trust_factor      = Decimal(str((score.trust_score      or 50.0) / 100.0)) if score else Decimal("0.5")
        confidence_factor = Decimal(str((score.confidence_weight or 0.5)))          if score else Decimal("0.5")
        payout = round(base_payout * trust_factor * confidence_factor, 6)
    else:
        payout = Decimal("0")
    ...
    await db.execute(
        ...
        .values(was_accepted=body.accepted, payout_amount=payout)
    )
    await db.commit()
